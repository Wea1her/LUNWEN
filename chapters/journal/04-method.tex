...
为支持结构消融实验，本文在实现中进一步提供两类可控开关：其一，关闭动作掩码（No-ActionMask）以检验合法性先验对策略学习的贡献；其二，关闭 \texttt{STOP} 动作（No-STOP）以检验主动终止能力对风险控制和打包效率的影响。完整模型保留动作掩码与 \texttt{STOP}，作为默认配置。No-ActionMask 变体额外记录非法动作次数、非法动作率、非法截断次数、非法截断率、最大连续非法动作长度和平均连续非法动作长度，用于解释移除掩码后的性能变化是否来自不可行动作探索成本。

\subsection{训练算法}

训练过程采用 PPO 的截断替代目标函数。在每个训练回合中，智能体与仿真环境交互完成一次完整的区块构建过程，收集轨迹 $\tau = \{(s_t, a_t, r_t, s_{t+1})\}_{t=0}^{T}$。即时奖励按第3章定义的分层奖励计算，包括单步收益--等待--风险引导、终止公平与风险约束，以及非法动作和 \texttt{STOP} 行为约束。使用广义优势估计（Generalized Advantage Estimation，GAE）计算优势函数：
...
训练阶段同时保存验证集选出的最优检查点 \texttt{best\_model.pt} 与末轮参数 \texttt{final\_model.pt}。具体做法是在训练开始前预生成固定验证交易池，每隔固定回合在该验证池上评估一次当前策略，并采用 V5 的 two-stage 选模规则选择最优模型：首先比较约束可行率，其次比较可行子集上的归一化区块收益，并以风险调整收益作为辅助排序项。该规则对应实现中的 \texttt{two\_stage\_selection\_score}，默认运行档位为 \texttt{balanced}。本文所有正式实验、鲁棒性实验与消融实验统一加载 \texttt{best\_model.pt} 进行评估，并在 \texttt{checkpoint\_meta.json}、\texttt{config\_snapshot.json} 与 \texttt{v5\_protocol\_manifest.json} 中记录验证池规模、评估间隔、最优回合、选模指标、约束阈值和运行档位，保证主表、附表与复现实验使用同一模型选择规则。
...
（4）重复上述过程直至满足终止条件：候选交易为空、不再存在满足 Gas 与 Nonce 约束的合法候选交易，或智能体选择 \texttt{STOP}。
...
（2）\textbf{No-ActionMask}：关闭合法性掩码机制，策略网络在全部候选交易（含不合法交易）上计算概率分布。当智能体选择不合法交易时，环境不执行该动作并施加负奖励惩罚；若非法动作触发提前截断，则记录 \texttt{invalid\_truncation\_count} 与 \texttt{invalid\_truncation\_rate}。该变体同时输出 \texttt{invalid\_action\_count}、\texttt{invalid\_action\_rate}、\texttt{max\_invalid\_streak} 和 \texttt{mean\_consecutive\_invalid\_actions}，用于评估合法动作先验对策略学习效率、可行打包率和最终性能的影响。



（3）\textbf{No-STOP}：移除 \texttt{STOP} 动作，智能体只能在候选池耗尽或不存在合法候选交易时结束区块构建。该变体用于检验主动终止能力对风险控制和打包效率的贡献。
