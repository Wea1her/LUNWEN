...
训练阶段同时保存验证集选出的最优检查点 \texttt{best\_model.pt} 与末轮参数 \texttt{final\_model.pt}。具体做法是在训练开始前预生成固定验证交易池，每隔固定回合在该验证池上评估一次当前策略，按验证指标（默认为 \texttt{block\_fee\_mean}）选择最优模型。本文所有正式实验、鲁棒性实验与消融实验统一加载 \texttt{best\_model.pt} 进行评估，并在 \texttt{checkpoint\_meta.json} 中记录验证池规模、评估间隔、最优回合与选模指标，保证主表、附表与复现实验使用同一模型选择规则。
